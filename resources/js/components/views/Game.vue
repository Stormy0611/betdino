            ...
            <div class="row" v-if="!game.id.startsWith('external:')">
                ...
                <div class="col">
                    <div :class="`game-content game-content-${$route.params.id} game-type-local`">
                        <component ref="gameComponent" :is="!game.id.startsWith('external') ? $route.params.id : 'third-party'"></component>
                    </div>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            if(this.user.user.break && +new Date() < new Date(this.user.user.break).getTime()) {
                this.$router.push('/profile/responsibleGaming');
            }
        },
        mounted() {
