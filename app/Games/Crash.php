...
class Crash extends MultiplayerGame {
    ...
    public function nextGame() {
        ...
        $nonce = now()->timestamp;
        $seed = ProvablyFair::generateServerSeed();

        $this->state()->nonce($nonce);

        if(Modules::get($this, false)->isEnabled(Module::find("crash"))
            && mt_rand(1, 100) <= floatval(Modules::get($this, false)->get(Module::find("crash"), 'chance'))) {

            $dbGame = Game::create([
                'nonce' => $nonce
            ]);

            $seed = (new ModuleSeeder($this, false, null, $dbGame))->find(function(ProvablyFairResult $result) use ($dbGame) {
                return $this->isLoss($result, $dbGame, []);
            }, $seed);

            $dbGame->delete();
        }

        $this->state()->clientSeed($seed);
        $this->state()->serverSeed($seed);

        $provablyFair = new ProvablyFair($this, $seed);
        $provablyFair->setClientSeed($seed);
        $provablyFair->setNonce($nonce);

        $multiplier = $provablyFair->result()->result()[0];
