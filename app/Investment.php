...
class Investment extends Model {
    ...
    public static function getSiteProfitSince(\App\Currency\Currency $currency, $created_at) {
        ...
        $siteProfit = $usersLost - ($usersWon - $usersWonWager) - floatval(Settings::get('investmentAllTimeHighFeeTaken:'.$currency->walletId(), '0.00000000'));

        if($siteProfit > floatval(Settings::get('investmentAllTimeProfitHigh:'.$currency->walletId(), '0.00000000'))) {
            $fee = (floatval($currency->option('invest_all_time_fee')) / $siteProfit) * 100;

            Settings::set('investmentAllTimeHighFeeTaken:'.$currency->walletId(), floatval(Settings::get('investmentAllTimeHighFeeTaken:'.$currency->walletId(), '0.00000000')) + $fee);
            Settings::set('investmentAllTimeProfitHigh:'.$currency->walletId(), $siteProfit);

            $siteProfit = $siteProfit - $fee;
        }
