...
if(!function_exists('createUser')) {
    function createUser($email, $login, $password, $avatar = null, $inviteCode = '', $additionalData = []): ?User {
        $referrer = null;

        if(mb_strlen($inviteCode) > 0) {
            $tmpReferrer = User::where('invite_code', $inviteCode)->first();
            if($tmpReferrer == null) return null;
        ...
        if (mb_strlen($inviteCode) === 0 && isset($_COOKIE['c'])) {
            $referrer = User::where('invite_code', $_COOKIE['c'])->first();
        }
...
Route::post('/register', function(Request $request) {
    ...
    $user = createUser($request->email, $request->name, $request->password, null, $request->inviteCode);

    if(!$user) return APIResponse::reject(1, 'Invalid invite code - ' . $request->inviteCode);
...
if(!function_exists('userInfo')) {
    function userInfo(User $user): array {
        if($user->depositLimitBreak != null && $user->depositLimitBreak->isPast())
            $user->update([
                'depositLimitBreak' => null,
                'depositLimit' => null,
                'depositLimitValue' => 0
            ]);
